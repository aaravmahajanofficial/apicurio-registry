...
|
|`3.0.0`
|URL to the registry agent documentation
|`apicurio.a2a.agent.icon-url`
|`optional<string>`
|
|`3.0.0`
|URL to the registry agent icon
...
|`1.0`
|`3.0.0`
|A2A protocol version supported by the registry agent
...
|`2.4.3.Final`
|API date format (DEPRECATED - do not use)
|`apicurio.apis.date-format-timezone`
|`string`
|`UTC`
|`2.4.3.Final`
|API date format timezone (DEPRECATED - do not use)
...
|`apicurio.auth.role-source.kubernetes.group-mapping.admin`
|`optional<string>`
|
|`3.3.0`
|Comma-separated Kubernetes groups that map to sr-admin role _(experimental)_
|`apicurio.auth.role-source.kubernetes.group-mapping.developer`
|`optional<string>`
|
|`3.3.0`
|Comma-separated Kubernetes groups that map to sr-developer role _(experimental)_
|`apicurio.auth.role-source.kubernetes.group-mapping.readonly`
|`optional<string>`
|
|`3.3.0`
|Comma-separated Kubernetes groups that map to sr-readonly role _(experimental)_
...
|`apicurio.authn.kubernetes.api-audiences`
|`optional<string>`
|
|`3.3.0`
|Comma-separated list of API audiences for TokenReview validation. If empty, audience is not validated. _(experimental)_
|`apicurio.authn.kubernetes.cache-expiration`
|`int`
|`5`
|`3.3.0`
|TokenReview result cache expiration in minutes _(experimental)_
|`apicurio.authn.kubernetes.enabled`
|`boolean`
|`false`
|`3.3.0`
|Enable Kubernetes TokenReview authentication _(experimental)_
|`apicurio.authn.mechanism.priority`
|`string`
|`basic,proxy-header,oidc`
|`3.2.3`
|Comma-separated ordered list of authentication mechanism names. Only mechanisms that are also enabled will be used. Valid values: basic, proxy-header, oidc, kubernetes.
...
|`apicurio.http-caching.opaque-etags-enabled`
|`optional<boolean>`
|
|`3.2.0`
|Hash raw ETag values before adding them to the header. Enabling this feature might marginally increase security, while disabling is useful for testing and debugging. If no value is provided, the feature is enabled when the `prod` profile is active.
|`apicurio.http-caching.vary-headers`
|`list<string>`
...
|Available from
|Description
|`apicurio.gitops.repo.branch`
|`optional<string>`
|
|`3.2.0`
|Branch to read from. This is a shorthand for single-repo setups. For multiple repositories, use the indexed apicurio.gitops.repos.N.branch properties instead. _(experimental)_
|`apicurio.gitops.repo.dir`
|`optional<string>`
|
|`3.2.0`
|Directory name of the Git repository, relative to the workspace. This is a shorthand for single-repo setups. For multiple repositories, use the indexed apicurio.gitops.repos.N.dir properties instead. _(experimental)_
|`apicurio.gitops.validate.enabled`
|`boolean`
|`true`
|`3.3.0`
|Enable the dry-run validation endpoints and the sidecar validation watch loop. When disabled, validation endpoints return HTTP 503. _(experimental)_
|`apicurio.gitops.validate.fetch.timeout.seconds`
|`int`
|`120`
|`3.3.0`
|Timeout for the sidecar to fetch a git ref during dry-run validation, in seconds. _(experimental)_
|`apicurio.gitops.validate.max-tasks`
|`int`
|`5`
|`3.3.0`
|Maximum number of validation tasks with disk resources (submitted, fetching, validating, or awaiting cleanup) before new requests are queued. Prevents disk exhaustion from too many concurrent checkouts. _(experimental)_
|`apicurio.gitops.validate.task.ttl.seconds`
|`int`
|`3600`
|`3.3.0`
|Time-to-live for dry-run validation tasks in seconds. Tasks are automatically removed after this period. _(experimental)_
|`apicurio.gitops.workspace`
|`string`
|`/repos`
|`3.2.0`
|Base directory where Git repositories are mounted. Repository directories are resolved relative to this path. _(experimental)_
|===
...
|Available from
|Description
|`apicurio.http.ssl.cipher-suites`
|`optional<string>`
|
|`3.3.1`
|Comma-separated list of enabled cipher suites (e.g. TLS_AES_256_GCM_SHA384,TLS_AES_128_GCM_SHA256). When not set, the JVM/Quarkus defaults apply. Invalid values will cause TLS handshake failures at runtime.
|`apicurio.http.ssl.protocols`
|`optional<string>`
|
|`3.3.1`
|Comma-separated list of enabled SSL/TLS protocol versions (e.g. TLSv1.2,TLSv1.3). When not set, the JVM/Quarkus defaults apply. Invalid values will cause TLS handshake failures at runtime.
|===
...
|`false`
|`3.0.0`
|Enable the Iceberg REST Catalog API _(experimental)_
|`apicurio.iceberg.warehouse`
|`optional<string>`
|
|`3.0.0`
|Default warehouse location for Iceberg tables
|`apicurio.metrics.iceberg.enabled`
|`boolean`
|`true`
|`3.0.0`
|Enable Iceberg-specific metrics collection
|===
...
== kubernetesops
.kubernetesops configuration options
[.table-expandable,width="100%",cols="6,3,2,3,5",options="header"]
|===
|Name
|Type
...
|Available from
|Description
|`apicurio.kubernetesops.label.registry-id`
|`string`
|`apicurio.io/registry-id`
|`3.0.0`
|Label key used to identify ConfigMaps belonging to this registry instance. The label value must match `apicurio.polling-storage.id`. _(experimental)_
|`apicurio.kubernetesops.namespace`
|`optional<string>`
|
|`3.0.0`
|Kubernetes namespace to watch for ConfigMaps. If not specified, auto-detects the pod's namespace or falls back to "default". _(experimental)_
|`apicurio.kubernetesops.watch.enabled`
|`boolean`
|`true`
|`3.0.0`
|Enable Kubernetes Watch API for real-time ConfigMap change detection. When enabled, changes are detected immediately via watch events in addition to periodic polling. _(experimental)_
|`apicurio.kubernetesops.watch.reconnect-delay`
|`duration`
|`PT10S`
|`3.0.0`
|Base delay before reconnecting after watch failure. Uses exponential backoff with a maximum of 5 minutes. _(experimental)_
|===
...
|`apicurio.rest.legacy-error-codes.enabled`
|`boolean`
|`false`
|`3.3.0`
|When enabled, legacy HTTP error codes are used (e.g. 409 instead of 400 for rule violations)
...
|`apicurio.webhooks.allow-insecure-urls`
|`boolean`
|`false`
|`3.3.0`
|Allow http:// webhook endpoint URLs (development only) _(experimental)_
|`apicurio.webhooks.delivery.auto-disable-threshold`
|`int`
|`50`
|`3.3.0`
|Consecutive delivery failures before auto-disabling a subscription _(experimental)_
|`apicurio.webhooks.delivery.backoff-multiplier`
|`double`
|`2.0`
|`3.3.0`
|Exponential backoff multiplier for webhook delivery retries _(experimental)_
|`apicurio.webhooks.delivery.batch-size`
|`int`
|`50`
|`3.3.0`
|Number of webhook deliveries claimed per worker poll _(experimental)_
|`apicurio.webhooks.delivery.concurrency`
|`int`
|`10`
|`3.3.0`
|Maximum concurrent webhook HTTP deliveries per registry instance _(experimental)_
|`apicurio.webhooks.delivery.http-timeout`
|`string`
|`15s`
|`3.3.0`
|HTTP timeout for webhook delivery requests _(experimental)_
|`apicurio.webhooks.delivery.in-progress-timeout`
|`string`
|`5m`
|`3.3.0`
|Duration after which in-progress webhook deliveries are reclaimed _(experimental)_
|`apicurio.webhooks.delivery.initial-delay`
|`string`
|`1s`
|`3.3.0`
|Initial retry delay for failed webhook deliveries _(experimental)_
|`apicurio.webhooks.delivery.max-attempts`
|`int`
|`10`
|`3.3.0`
|Maximum delivery attempts before moving a webhook delivery to dead-letter status _(experimental)_
|`apicurio.webhooks.delivery.max-delay`
|`string`
|`5m`
|`3.3.0`
|Maximum retry delay for failed webhook deliveries _(experimental)_
|`apicurio.webhooks.delivery.max-inflight-per-subscription`
|`int`
|`3`
|`3.3.0`
|Maximum in-flight webhook deliveries per subscription _(experimental)_
|`apicurio.webhooks.delivery.poll-every`
|`string`
|`2s`
|`3.3.0`
|How often the webhook delivery worker polls for pending deliveries _(experimental)_
|`apicurio.webhooks.delivery.shutdown-timeout`
|`string`
|`30s`
|`3.3.0`
|Grace period to drain in-flight webhook deliveries on shutdown _(experimental)_
|`apicurio.webhooks.enabled`
|`boolean`
|`false`
|`3.3.0`
|Enable CloudEvents webhook notifications _(experimental)_
|`apicurio.webhooks.fanout.reconcile-every`
|`string`
|`30s`
|`3.3.0`
|How often the webhook fanout reconciler polls for pending fanouts _(experimental)_
|`apicurio.webhooks.log.retention`
|`string`
|`30d`
|`3.3.0`
|Retention period for webhook delivery audit logs _(experimental)_
|`apicurio.webhooks.payload.max-bytes`
|`int`
|`262144`
|`3.3.0`
|Maximum CloudEvent payload size in bytes before truncation _(experimental)_
|`apicurio.webhooks.rule-violations.enabled`
|`boolean`
|`true`
|`3.3.0`
|Emit rule.violated CloudEvents when artifact writes are rejected by rules _(experimental)_
|`apicurio.webhooks.security.block-private-ips`
|`boolean`
|`true`
|`3.3.0`
|Block webhook URLs that resolve to private or link-local IP addresses _(experimental)_
|`apicurio.webhooks.subscriptions.max-count`
|`int`
|`100`
|`3.3.0`
|Maximum number of webhook subscriptions per registry instance _(experimental)_
|`apicurio.webhooks.violations.max-count`
|`int`
|`20`
|`3.3.0`
|Maximum rule violations included in a rule.violated CloudEvent _(experimental)_
|===
...
|`false`
|`3.2.0`
|Enable search indexing _(experimental)_
|===
...
|`registry-events`
|`3.0.1`
|Kafka sql storage events topic
|`apicurio.events.kafka.topic.*`
|`map<string, string>`
|
|`3.1.3`
...
|`apicurio.polling-storage.debounce.max-wait-period`
|`duration`
|`PT90S`
|`3.2.0`
|Maximum time to wait for changes to settle before forcing a load. Prevents indefinite delay when changes keep arriving. Set to PT0S for no maximum. _(experimental)_
|`apicurio.polling-storage.debounce.quiet-period`
|`duration`
|`PT3S`
|`3.2.0`
|After detecting a change, wait until no new changes are seen for this duration before loading data. Absorbs rapid successive pushes. Set to PT0S to disable debouncing. _(experimental)_
|`apicurio.polling-storage.deterministic-ids-enabled`
|`boolean`
|`true`
|`3.2.0`
|When enabled, contentId and globalId are deterministically generated from content hashes and artifact coordinates if not explicitly specified. When disabled, missing IDs cause an error. Generated IDs use the upper half of positive longs (above Long.MAX_VALUE/2), leaving the lower half for user-specified IDs. _(experimental)_
|`apicurio.polling-storage.file-suffixes`
|`list<string>`
|`registry,ar`
|`3.2.0`
|Comma-separated list of file suffixes that identify registry metadata files. Each suffix is combined with supported extensions (.yaml, .yml, .json) to form the full pattern. For example, suffix "registry" matches *.registry.yaml, *.registry.yml, and *.registry.json. _(experimental)_
|`apicurio.polling-storage.id`
|`string`
|`main`
|`3.2.0`
|Identifier of this Registry instance. Only data that references this identifier will be loaded. _(experimental)_
|`apicurio.polling-storage.poll-period`
|`duration`
|`PT10S`
|`3.2.0`
|Minimum period between data source polls. The actual poll (reading from Git or Kubernetes) only happens when at least this much time has passed since the last poll. _(experimental)_
|`apicurio.polling-storage.require-registry-config`
|`boolean`
|`true`
|`3.2.0`
...
|`7`
|`3.3.0`
|Number of days within which a schema is classified as ACTIVE
|`apicurio.usage.aggregation.every`
|`string`
|`1h`
|`3.3.0`
|How often the usage metrics OTel gauges are updated (Quarkus scheduler expression)
|`apicurio.usage.dead-threshold-days`
|`int`
|`90`
|`3.3.0`
|Number of days after which a schema is classified as DEAD
|`apicurio.usage.drift-alert-threshold`
|`int`
|`2`
|`3.3.0`
|Number of versions behind latest before triggering a drift alert
|`apicurio.usage.retention-days`
|`int`
|`180`
|`3.3.0`
|Number of days to retain raw usage events before cleanup
|`apicurio.usage.stale-threshold-days`
|`int`
|`30`
|`3.3.0`
|Number of days after which a schema is classified as STALE
|`apicurio.usage.telemetry.enabled`
|`boolean`
|`false`
|`3.3.0`
|Enable usage telemetry collection from SerDes clients _(experimental)_
|===
